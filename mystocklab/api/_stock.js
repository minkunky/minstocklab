...
export async function searchUS(query) {
  ...
  return (data.result || [])
    // Common Stock + ETF/ETP 허용, 점/하이픈/콜론 포함 심볼 제외
    .filter(r => ['Common Stock', 'ETP', 'ETF'].includes(r.type) && !/[.\-:]/.test(r.symbol))
    .slice(0, 10)
    .map(r => ({
      // Finnhub이 "AAPL:1" 같은 거래소 접미사를 붙이는 경우 제거
 ...
 */
export async function quoteUSFinnhub(symbol) {
  const key = process.env.FINNHUB_API_KEY
  if (!key) throw new Error('FINNHUB_API_KEY not set')
  ...
  // Content-Type 확인 — HTML이 오면(rate limit 등) JSON 파싱 전에 차단
  const ct = quoteRes.headers.get('content-type') ?? ''
  if (!ct.includes('application/json')) {
    throw new Error(`Finnhub returned non-JSON (${ct}) for ${symbol}`)
  }

  const q = await quoteRes.json()
  if (q.c == null) throw new Error(`No price data for ${symbol}`)

  let name = symbol
  if (profileRes.ok) {
    const pct = profileRes.headers.get('content-type') ?? ''
  ...
  // 장 마감 시 Finnhub은 d/dp를 0으로 내려주므로, c와 pc로 직접 계산
  const change        = q.pc ? q.c - q.pc : (q.d ?? 0)
  const changePercent = q.pc ? ((q.c - q.pc) / q.pc) * 100 : (q.dp ?? 0)
...
/**
 * Yahoo Finance — 미국 주식 폴백
 */
export async function quoteUSYahoo(symbol) {
  const url  = `https://query1.finance.yahoo.com/v8/finance/chart/${symbol}?interval=1d&range=1d`
  const res  = await fetch(url, {
    headers: { 'User-Agent': 'Mozilla/5.0 (compatible; MyStockLab/1.0)' },
  })
  if (!res.ok) throw new Error(`Yahoo US ${res.status} for ${symbol}`)
  const data = await res.json()
  const meta = data?.chart?.result?.[0]?.meta
  if (!meta?.regularMarketPrice) throw new Error(`Yahoo US: no price for ${symbol}`)
...
/**
 * 미국 주식 시세 (Finnhub 우선, 실패 시 Yahoo 폴백)
 */
export async function quoteUS(symbol) {
  // 거래소 접미사 제거 (예: "AAPL:1" → "AAPL")
  const cleanSymbol = symbol.split(':')[0].toUpperCase()
  try {
    return await quoteUSFinnhub(cleanSymbol)
  } catch (err) {
    console.warn(`[quote] Finnhub failed (${cleanSymbol}): ${err.message} → Yahoo fallback`)
    return await quoteUSYahoo(cleanSymbol)
  }
}
...
export async function getQuote(symbol) {
  // 거래소 접미사 안전망 제거 (예: "AAPL:1" → "AAPL")
  const s = symbol.trim().toUpperCase().split(':')[0]
